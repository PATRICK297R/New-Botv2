...
Susunlah kata berikut ini dengar benar:
${json.soal}
...
`.trim()
    conn.susunkata[id] = [
        await conn.sendButton(m.chat, caption, 'PatrickBotz', 'Bantuan', '.suka', m),
        json, poin,
        setTimeout(async () => {
            if (conn.susunkata[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.jawaban}*`, 'PatrickBotz', 'Susun Kata', '.susunkata', conn.susunkata[id][0])
