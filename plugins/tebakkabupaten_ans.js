...
handler.before = async function (m) {
    ...
    if (m.quoted.id == this.tebakkabupaten[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.title.toLowerCase().trim()) {
            ...
            global.db.data.users[m.sender].dailyReward += 1
            await this.sendButton(m.chat, `*Benar!* +${this.tebakkabupaten[id][2]} XP + 1 dailyReward`, watermark, 'Tebak Kabupaten', '.tebakkabupaten', m)
