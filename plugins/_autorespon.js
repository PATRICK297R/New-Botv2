    ...
    try {
        if (m.mentionedJid.includes(this.user.jid) && m.isGroup) {
            await this.send2Button(m.chat,
                isBanned ? 'Bot Sedang Di Banned di chat ini, Hanya member premium yang bisa command' : banned ? 'kamu dibanned' : 'Iyah, Ngfain tag gwehj☝️😅',
                '',
...
╠═〘 R-TXZY〙 ═`.trim(), '© R-Txzy', 'Owner', '.owner', 'Pulsa Im3', '#viapulsa', 'Scan All Payment', '.scanall', m)
    ...
    if (new Date() * 1 - setting.status > 1000) {
        ...
        await this.setStatus(`ONLINE: ${uptime}`).catch(_ => _)
